	...
	for _, tc := range testCases {
		t.Run(tc.Title, func(t *testing.T) {
			for _, tv := range tc.TestVectors {
				...
				require.NoError(t, err)
				require.Equal(t, messages.ReturnValue_OK, digestResponse.(*messages.DigestResponse).ReturnValue)
				require.Equal(t, strings.ReplaceAll(tv.Digest, " ", ""), hex.EncodeToString(digestResponse.(*messages.DigestResponse).Digest))
			}
		})
	}
}
		...
		t.Run(tc.Title, func(t *testing.T) {
			for tvId, tv := range tc.TestVectors {
				...
				for _, chunk := range chunks {
					...
					require.NoError(t, err)
					require.Equal(t, messages.ReturnValue_OK, digestUpdateResponse.(*messages.DigestUpdateResponse).ReturnValue)
				}
...
func processSlipMsg(srv servers.Server, req messages.Message) (messages.Message, error) {
	request, err := messages.Marshall(req)
	if err != nil {
		return nil, err
	}

	response, err := srv.Process(request)
	if err != nil {
		return nil, err
	}

	return messages.Unmarshal(response)
}
